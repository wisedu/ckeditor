...
(function() {
	...
	CKEDITOR.plugins.add('toolbar', {
		...
		init: function(editor) {
			...
			if (sessionStorage.pageSize != '394') {
				editor.ui.addRichCombo('Pagesize', {
					label: '页面尺寸',
					title: '页面尺寸',
					toolbar: 'forms,10',
					...
					init: function() {
						...
						pagesizeArr.forEach(function(item) {
							var label = item.name;
							that.add(item.name, "<p>" + label + "</p>", label);
						});
					},
			...
			addCmd('rowAddcolumns', createDef({
				requiredContent: 'table',
				exec: function(editor, value) {
					var selection = editor.getSelection(),
						cells = getSelectedCells(selection);

					insertRow(cells, false, value, selection);
				}
			}));
			// 增加插入一行几列的选项
			editor.ui.addRichCombo('Addcolumns', {
				label: '插入列类型',
				title: '插入列类型',
				toolbar: 'forms,20',
				...
				init: function() {
					var that = this;
					if (sessionStorage.appType === 'mobile' || sessionStorage.appType === 'MOBILE') {
						var pagesizeArr = [{
							name: '一行两列',
							id: '1r2c'
						}];
					...
					pagesizeArr.forEach(function(item) {
						var label = item.name;
						that.add(item.id, "<p>" + label + "</p>", label);
					});
				},

				onClick: function(value) {
					editor.focus();
					editor.execCommand('rowAddcolumns', value);
					//window.parent.postMessage({
					//	'type': 'ybt_addColumns',
					//	'value': value
					//}, '*');
	...
	//添加一行几列的表格
	function insertRow(selectionOrCells, insertBefore, type, document) {
		if (selectionOrCells.length > 0) {
		...
		newTable.addClass('wistable');
		newTable.addClass('a4');
		newTable.setAttribute('border', '1');
		var newRow = doc.createElement('tr');
		for (var i = 0; i < width; i++) {
			var cell;
			cell = doc.createElement('td');
			//如果是标题头，就设置宽度。如果是内容td，添加br标签
			if (i % 2 === 0) {
		...
		if (!table) {
			var ranges = document.getRanges();
			if (ranges[0].collapsed) {
				// Walker does not handle collapsed ranges yet - fall back to old API.
				table = ranges[0].getCommonAncestor();
			}
		}
		// insertBefore ? newRow.insertBefore(row) : newRow.insertAfter(row);
		insertBefore ? newTable.insertBefore(table) : newTable.insertAfter(table);
	...
	function getSelectedCells(selection, table) {
		...
		for (var i = 0; i < ranges.length; i++) {
			...
			if (range.collapsed) {
				...
				var nearestCell = startNode.getAscendant({
					td: 1,
					th: 1
				}, true);
	...
	function getPrivateToolbarGroups(editor) {
		return editor._.toolbarGroups || (editor._.toolbarGroups = [{
			...
			{
				name: 'basicstyles',
				groups: ['undo', 'basicstyles', 'cleanup']
			}, {
				name: 'forms'
			}, {
				name: 'paragraph',
