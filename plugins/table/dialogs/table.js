		...
		return {
			...
			onOk: function() {
				...
				if (sessionStorage.appType === 'mobile' || sessionStorage.appType === 'MOBILE') {
					table.addClass('wistable-mobile');
				} else {
					table.addClass('wistable');
					table.addClass('a4');
				}
				if ( data.info ) {
					var info = data.info;
					...
					if ( !this._.selectedElement ) {
						if (sessionStorage.appType === 'mobile' || sessionStorage.appType === 'MOBILE') {
							...
							var thead_row = thead.append( makeElement( 'tr' ) );
							var thead_td = thead_row.append( makeElement( 'td' ) );
							thead_td.setAttribute('colspan',2);
							thead_td.appendHtml("<div>分组标题</div>");
						}
						//添加tbody
					...
					if ( table.$.tHead !== null && !( headers == 'row' || headers == 'both' ) && (table.$.className.indexOf('docgrid')===-1)) {
				...
				if ( !this._.selectedElement ) {
					...
					setTimeout( function() {
						...
						if (tableParent.className.indexOf('ybt_clearfix') === -1) {
							tableParent.className +=' ybt_clearfix';
						}
