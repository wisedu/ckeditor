...
( function() {
	...
	function initElementsPath( editor, bottomSpaceData ) {
		...
		function onClick( elementIndex ) {
			...
			//添加点击底部选择器时,让被选中的容器可进行属性设置
			var selectDom = element.$.offsetParent.querySelector('[turing-element-select="active"]');
			if(selectDom){
				selectDom.setAttribute('turing-element-select', 'blur');
			}
			element.$.setAttribute('turing-element-select', 'active');
			parent.window.postMessage({
				type: 'showNoComponentProperty'
			},'/');
