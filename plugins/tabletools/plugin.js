...
( function() {
	...
	CKEDITOR.plugins.tabletools = {
		...
		init: function( editor ) {
			...
			addCmd( 'cellborderBottom', createDef( {
				requiredContent: 'table',
				exec: function( editor ) {
					var selection = editor.getSelection(),
						cursorPosition = getSelectedCells( selection );
					if (cursorPosition.length>0) {
						cursorPosition.forEach(function(item){
							if(item.$.style.borderBottomStyle){
								if (item.$.style.borderBottomStyle === 'none') {
			...
			addCmd( 'cellborderTop', createDef( {
				requiredContent: 'table',
				exec: function( editor ) {
					var selection = editor.getSelection(),
						cursorPosition = getSelectedCells( selection );
					if (cursorPosition.length>0) {
						cursorPosition.forEach(function(item){
							if(item.$.style.borderTopStyle){
								if (item.$.style.borderTopStyle === 'none') {
									item.$.style.borderTopStyle = 'solid';
								} else {
									item.$.style.borderTopStyle = 'none';
								}
							}else {
								item.$.style.borderTopStyle ='solid';
			...
			addCmd( 'cellborderLeft', createDef( {
				requiredContent: 'table',
				exec: function( editor ) {
					var selection = editor.getSelection(),
						cursorPosition = getSelectedCells( selection );
					if (cursorPosition.length>0) {
						cursorPosition.forEach(function(item){
							if(item.$.style.borderLeftStyle){
								if (item.$.style.borderLeftStyle === 'none') {
									item.$.style.borderLeftStyle = 'solid';
								} else {
									item.$.style.borderLeftStyle = 'none';
								}
							}else {
								item.$.style.borderLeftStyle ='solid';
								item.$.style.borderLeftWidth ='1px';
								item.$.style.borderLeftColor ='inherit';
							}
						})
					}
				}
			} ) );
			addCmd( 'cellborderRight', createDef( {
				requiredContent: 'table',
				exec: function( editor ) {
					var selection = editor.getSelection(),
						cursorPosition = getSelectedCells( selection );
					if (cursorPosition.length>0) {
						cursorPosition.forEach(function(item){
							if(item.$.style.borderRightStyle){
								if (item.$.style.borderRightStyle === 'none') {
									item.$.style.borderRightStyle = 'solid';
								} else {
									item.$.style.borderRightStyle = 'none';
								}
							}else {
								item.$.style.borderRightStyle ='solid';
								item.$.style.borderRightWidth ='1px';
								item.$.style.borderRightColor ='inherit';
							}
						})
					}
				}
			...
			if ( editor.addMenuItems ) {
				editor.addMenuItems( {
					...
					//增加右键菜单中的单元格边框
					tablecellborder: {
						label: lang.cellborder.menu,
						group: 'tablecellborder',
						order: 1,
						getItems: function() {
							return {
								tablecellborder_top: CKEDITOR.TRISTATE_OFF,
								tablecellborder_bottom: CKEDITOR.TRISTATE_OFF,
								tablecellborder_left: CKEDITOR.TRISTATE_OFF,
								tablecellborder_right: CKEDITOR.TRISTATE_OFF
								
							};
						}
					},
					tablecellborder_top: {
						label: lang.cellborder.top,
						group: 'tablecellborder',
						command: 'cellborderTop',
						order: 5
					},
					tablecellborder_bottom: {
						label: lang.cellborder.bottom,
						group: 'tablecellborder',
						command: 'cellborderBottom',
						order: 5
					},
					tablecellborder_left: {
						label: lang.cellborder.left,
						group: 'tablecellborder',
						command: 'cellborderLeft',
						order: 5
					},
					tablecellborder_right: {
						label: lang.cellborder.right,
						group: 'tablecellborder',
						command: 'cellborderRight',
						order: 5
					},
				...
				editor.contextMenu.addListener( function( element, selection, path ) {
					...
					if ( cell && !cell.isReadOnly() ) {
						return {
							...
							tablecellborder: CKEDITOR.TRISTATE_OFF
						};
					}
