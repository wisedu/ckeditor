	...
	CKEDITOR.loader = ( function() {
		...
		return {
			...
			loadAllScript: function(){
				...
				this.loadScript(allScript, 0);

			},
			loadScript: function(allScript, index, isReload){
				clearTimeout(scriptLoadTime);
				var scriptName = allScript[index];
				if(!scriptName){
					return;
				}
				var that = this;
				...
				if ( ( 's:' + scriptName ) in this.loadedScripts && !isReload){
					this.loadScript(allScript, index);
					return;
				}
				...
				script.onload = function(){
					clearTimeout(scriptLoadTime);
					that.loadScript(allScript, index);
				};
				...
				scriptLoadTime = setTimeout(function(){
					if(isReload){
						that.loadScript(allScript, index);
					}else{
						that.loadScript(allScript, index-1, true);
					}
				}, 20000);
			},
			removeScript: function(src){
				var jsDom = document.querySelector('[src="'+src+'"]');
				if(jsDom){
					var parentDom = jsDom.parentNode;
					parentDom.removeChild(jsDom);
				}
			}
		};
	} )();
